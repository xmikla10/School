    ...
    for( y = miny; y <= maxy; ++y )
    {
        ...
        for( x = minx; x <= maxx; ++x )
        {
            if( e1 >= 0 && e2 >= 0 && e3 >= 0 )
            {
                ...
                /* interpolace barvy */
                clr.red = clr.red/255.0;
                clr.green = clr.green/255.0;
                clr.blue = clr.blue/255.0;

                color.red = ROUND2BYTE((w1 * col1.red + w2 * col2.red + w3 * col3.red ) * clr.red);
                color.green = ROUND2BYTE((w1 * col1.green + w2 * col2.green + w3 * col3.green ) * clr.green);
                color.blue = ROUND2BYTE((w1 * col1.blue + w2 * col2.blue + w3 * col3.blue) * clr.blue);
                color.alpha = 255.0;
...
S_RGBA studrenTextureValue( S_StudentRenderer * pRenderer, double u, double v )
{
	...
	double red = (pRenderer->textura[pRenderer->sirka*y + x].red * u_opposite  + pRenderer->textura[pRenderer->sirka*y + x + 1].red   * u_ratio) * v_opposite + 
                   (pRenderer->textura[pRenderer->sirka*(y+1) + x].red * u_opposite  + pRenderer->textura[pRenderer->sirka*(y+1) + x + 1].red * u_ratio) * v_ratio;

	double green = (pRenderer->textura[pRenderer->sirka*y + x].green * u_opposite  + pRenderer->textura[pRenderer->sirka*y + x + 1].green   * u_ratio) * v_opposite + 
                   (pRenderer->textura[pRenderer->sirka*(y+1) + x].green * u_opposite  + pRenderer->textura[pRenderer->sirka*(y+1) + x + 1].green * u_ratio) * v_ratio;
          
	double blue = (pRenderer->textura[pRenderer->sirka*y + x].blue * u_opposite  + pRenderer->textura[pRenderer->sirka*y + x + 1].blue   * u_ratio) * v_opposite + 
                   (pRenderer->textura[pRenderer->sirka*(y+1) + x].blue * u_opposite  + pRenderer->textura[pRenderer->sirka*(y+1) + x + 1].blue * u_ratio) * v_ratio;   
