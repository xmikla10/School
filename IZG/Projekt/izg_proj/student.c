...
float pom = 100;
int i = 1;
const S_Material    MAT_WHITE_AMBIENT  = { 1.0, 1.0, 0.0, 1.0 };
...
S_Renderer * studrenCreate()
{
    ...
    //renderer->texture = (S_RGBA*) malloc(sizeof(S_RGBA) * 512 * 512);
...
void studrenDrawTriangle(S_Renderer *pRenderer,
                         ...
                         int x2, int y2,
                         int x3, int y3,
                         S_Triangle  * triangle, double hom1, double hom2, double hom3
                         )
{
    ...
    for( y = miny; y <= maxy; ++y )
    {
        ...
        for( x = minx; x <= maxx; ++x )
        {
            if( e1 >= 0 && e2 >= 0 && e3 >= 0 )
            {
                ...
                //double u = w1 * triangle->t[0].x + w2 * triangle->t[1].x + w3 * triangle->t[2].x;
                //double v = w1 * triangle->t[0].y + w2 * triangle->t[1].y + w3 * triangle->t[2].y;

				double u = (w1 * (triangle->t[0].x / hom1) + w2 * (triangle->t[1].x / hom2) + w3 * (triangle->t[2].x / hom3))
							/ (w1*(1 / hom1)+w2 * (1 / hom2) + w3 * (1 / hom3));
                double v = (w1 * (triangle->t[0].y / hom1) + w2 * (triangle->t[1].y / hom2) + w3 * (triangle->t[2].y / hom3))
							/ (w1*(1 / hom1)+w2 * (1 / hom2) + w3 * (1 / hom3));

                S_RGBA clr = studrenTextureValue((S_StudentRenderer*)pRenderer, u, v);
...
S_Coords interpolace( S_Coords *first, S_Coords *second, float t)
{
	S_Coords pom;

    pom.x = ( 1 - t ) * first->x + (t * second->x);
    pom.y = ( 1 - t ) * first->y + (t * second->y);
    pom.z = ( 1 - t ) * first->z + (t * second->z);
...
void studrenProjectTriangle(S_Renderer *pRenderer, S_Model *pModel, int i, float n)
{
    ...
    int         i0_2, i1_2, i2_2, in_2;
    S_Coords    i0N, i1N, i2N, inN;
    S_Coords    i0N1, i1N1, i2N1;
    float 		t;
    ...
    i0_2 = triangle->v[ 0 ] + vertexOffset2;
    i1_2 = triangle->v[ 1 ] + vertexOffset2;
    i2_2 = triangle->v[ 2 ] + vertexOffset2;

    /* index normaloveho vektoru pro i-ty trojuhelnik n-teho snimku - pricteni offsetu */
    in = triangle->n + normalOffset;
    in_2 = triangle->n + normalOffset2;

    t = n - (int)n;

    i0N = interpolace( cvecGetPtr(pModel->vertices, i0), cvecGetPtr(pModel->vertices, i0_2), t);
    i1N = interpolace( cvecGetPtr(pModel->vertices, i1), cvecGetPtr(pModel->vertices, i1_2), t);
   	i2N = interpolace( cvecGetPtr(pModel->vertices, i2), cvecGetPtr(pModel->vertices, i2_2), t);
    ...
    trTransformVertex(&aa, &i0N);
    trTransformVertex(&bb, &i1N);
    trTransformVertex(&cc, &i2N);

    /* promitneme vrcholy trojuhelniku na obrazovku */
    double hom1 = trProjectVertex(&u1, &v1, &aa);
    double hom2 = trProjectVertex(&u2, &v2, &bb);
    double hom3 = trProjectVertex(&u3, &v3, &cc);

    i0N1 = interpolace( cvecGetPtr(pModel->normals, i0), cvecGetPtr(pModel->normals, i0_2), t);
    i1N1 = interpolace( cvecGetPtr(pModel->normals, i1), cvecGetPtr(pModel->normals, i1_2), t);
   	i2N1 = interpolace( cvecGetPtr(pModel->normals, i2), cvecGetPtr(pModel->normals, i2_2), t);
    ...
    trTransformVector(&naa, &i0N1);
    trTransformVector(&nbb, &i1N1);
    trTransformVector(&ncc, &i2N1);
    ...
    inN = interpolace( cvecGetPtr(pModel->trinormals, in), cvecGetPtr(pModel->trinormals, in_2), t);
    
    trTransformVector(&nn, cvecGetPtr(pModel->trinormals, in));
    trTransformVector(&nn, &inN);
...
S_RGBA studrenTextureValue( S_StudentRenderer * pRenderer, double u, double v )
{
	...
	int x = floor(v);
   	...
   	double vOpposite = 1 - v_ratio;

	double red = (pRenderer->textura[pRenderer->sirka*y + x].red * uOpposite  + pRenderer->textura[pRenderer->sirka*y + x + 1].red   * uRatio) * vOpposite + 
                   (pRenderer->textura[pRenderer->sirka*(y+1) + x].red * uOpposite  + pRenderer->textura[pRenderer->sirka*(y+1) + x + 1].red * uRatio) * v_ratio;

	double green = (pRenderer->textura[pRenderer->sirka*y + x].green * uOpposite  + pRenderer->textura[pRenderer->sirka*y + x + 1].green   * uRatio) * vOpposite + 
                   (pRenderer->textura[pRenderer->sirka*(y+1) + x].green * uOpposite  + pRenderer->textura[pRenderer->sirka*(y+1) + x + 1].green * uRatio) * v_ratio;
          
	double blue = (pRenderer->textura[pRenderer->sirka*y + x].blue * uOpposite  + pRenderer->textura[pRenderer->sirka*y + x + 1].blue   * uRatio) * vOpposite + 
                   (pRenderer->textura[pRenderer->sirka*(y+1) + x].blue * uOpposite  + pRenderer->textura[pRenderer->sirka*(y+1) + x + 1].blue * uRatio) * v_ratio;   
